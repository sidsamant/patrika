...
def _is_twitter_target(raw_url: Any) -> bool:
    """Return whether a URL points to X/Twitter and should be included."""
...
def _artifact_key(scraped_item_id: int) -> str:
    """Build the audit key used to track already-hoarded Crawl4AI rows."""
    return f"scraped_item:{scraped_item_id}"


def _normalize_scraped_row(source: SourceConfig, row: sqlite3.Row) -> dict[str, object] | None:
    """Convert one Crawl4AI X/Twitter row into the shared hoarder metadata schema."""
    item_id = int(row["id"])
    ...
    if not url or (not _is_twitter_target(url) and not _is_twitter_target(source_page)):
        return None
    ...
    return {
        ...
        "tags": [tag for tag in [section, item_type, "twitter"] if tag],
...
def _load_unread_scraped_rows(source: SourceConfig, db_path: Path) -> list[sqlite3.Row]:
    """Load unread X/Twitter Crawl4AI rows ordered by first-seen time."""
    ...
    unread_rows = [
        row
        for row in rows
        if _artifact_key(int(row["id"])) not in processed_keys
        and (_is_twitter_target(row["url"]) or _is_twitter_target(row["source_page"]) or _is_twitter_target(row["source_link"]))
    ]
    logger.debug(
        "Twitter source found %d unread scraped_items rows out of %d total in %s",
        len(unread_rows),
        len(rows),
        db_path,
    )
...
class TwitterHoarderAgent(BaseAgent):
    """Read Crawl4AI scraped_items rows and hoard X/Twitter entries."""

    def __init__(self, source: SourceConfig) -> None:
        """Initialize the twitter hoarder agent for one configured Crawl4AI DB."""
        super().__init__(
            name="twitter_hoarder",
            description="Custom hoarder agent that reads Crawl4AI scraped items and extracts X/Twitter entries.",
        )
        self._source = source
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        """Collect unread X/Twitter Crawl4AI items, merge them into session state, and audit processed rows."""
        db_path = self._db_path()
        logger.debug("Running twitter hoarder for source_id=%s db=%s", self._source.id, db_path)

        if not db_path.exists() or not db_path.is_file():
            raise FileNotFoundError(f"Twitter Crawl4AI database not found: {db_path}")
        ...
        for row in unread_rows:
            ...
            try:
                ...
                logger.debug("Twitter source ingested scraped_item id=%s", scraped_item_id)
                ...
                logger.exception("Twitter source failed reading scraped_item id=%s", scraped_item_id)
        ...
        logger.debug("Merged twitter results into session state. total_items=%d", len(merged_items))
...
def create_twitter_hoarder_agent(source: SourceConfig) -> BaseAgent:
    """Create a twitter hoarder agent for the configured Crawl4AI database."""
    return TwitterHoarderAgent(source)
