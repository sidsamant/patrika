...
NEWSLETTER_LOG_PATH = LOGS_DIR / f"newsletter-{RUN_TIMESTAMP}.debug.log"
...
def _configure_logging() -> logging.Logger:
    ...
    resolved_log_path = NEWSLETTER_LOG_PATH.resolve()
    if not any(
...
logger.debug("Initialized newsletter runner logging. log_path=%s", NEWSLETTER_LOG_PATH)
...
async def main() -> None:
    ...
    prompt = types.Content(
        role="user",
        parts=[types.Part(text="Generate the newsletter from available sectionizer outputs")],
    )
