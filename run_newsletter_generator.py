...
NEWSLETTER_GENERATOR_LOG_PATH = LOGS_DIR / f"newsletter-generator-{RUN_TIMESTAMP}.debug.log"
...
def _configure_logging() -> logging.Logger:
    ...
    resolved_log_path = NEWSLETTER_GENERATOR_LOG_PATH.resolve()
    if not any(
...
logger.debug("Initialized newsletter generator runner logging. log_path=%s", NEWSLETTER_GENERATOR_LOG_PATH)
...
async def main() -> None:
    logger.debug("Starting newsletter generator runner")
    logger.debug("Newsletter generator debug log path: %s", NEWSLETTER_GENERATOR_LOG_PATH)
